# Added charting functionality for expense categories from Melissa's A4 Demo
...
# MENU OPTION 4: VIEW CATEGORY CHART  (Jenny's NEW PART)
def view_category_chart(expenses):
    """
    Show a bar chart or pie chart of the expense categories using matplotlib.
    """
    # 1. Check if there is any spending at all
    if all(amount == 0 for amount in expenses.values()):
        print("\nNo expenses recorded yet. Add some transactions first.")
        return

    print("\n --- Category Chart ---")
    print("1. Bar Chart")
    print("2. Pie Chart")
    chart_choice = input("Choose chart type (1-2): ").strip()
